...
function buildCellNBTCellAPI(items, cellName, lore) {
    ...
    // 解析所有物品
    var parsed = items.map(parseItemStringCellAPI);
    ...
    // 生成amtsNBT - 使用有符号长整型(L后缀)，添加简化溢出检查
    var amtsNBT = parsed.map(function(item) {
        ...
        // 检查是否在JavaScript安全整数范围内 (±2^53-1)，确保精度
        var MAX_SAFE_INTEGER = Number.MAX_SAFE_INTEGER; // 2^53-1
        if (count > MAX_SAFE_INTEGER || count < -MAX_SAFE_INTEGER) {
            warn('[256k Cell API] 警告: 物品数量 ' + count + ' (来自物品: ' + item.id + ') 超出JavaScript安全整数范围，可能丢失精度');
        }
        
        // 简化版Java long范围检查（使用普通数字，注意精度问题）
        var MAX_LONG = 9223372036854775807; // 2^63-1
        var MIN_LONG = -9223372036854775808; // -2^63
        if (count > MAX_LONG || count < MIN_LONG) {
            warn('[256k Cell API] 警告: 物品数量 ' + count + ' (来自物品: ' + item.id + ') 超出 Java long 范围，可能无法正确存储');
        }
